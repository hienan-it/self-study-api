...
@router.get("/page/", response_model=Page[UserResponse])
async def page_users(
        page: int = Query(0, ge=0, description=""),
        page_size: int = Query(100, ge=1, le=1000, description=""),
        role: Optional[UserRole] = Query(None, description="Filter by role"),
        is_deleted: Optional[bool] = Query(None, description="Filter by active status"),
        search: Optional[str] = Query(None, description="Search by username or email"),
        current_user: User = Depends(get_teacher_or_admin),
    ...
    """
    Page all users with optional filters

    Requires: Teacher or Admin role
    """
    return user_service.page(
        page=page,
        page_size=page_size,
        role=role,
        is_deleted=is_deleted,
...
async def count_users(
    ...
    """
    return user_service.count_users(role=role, is_deleted=is_deleted)
